# **V0004 Golden Waveform Template**  
### Asymmetric lattice, zero warp, zero error — FALSE case  
*(Informative)*

This document defines the expected waveform characteristics for **Conformance Vector V0004**, which introduces a deliberate **asymmetry** in the phase lattice while maintaining zero warp and zero error. This vector validates that implementations correctly detect asymmetry and produce a **FALSE** result even when all other conditions are nominal.
...
| `warp_sum` | `(0, 0, 0)` |
| `error_sum` | `0` |
| `symmetry_ok` | `0` (asymmetry detected) |
| `error_ok` | `1` |
| `true_delivery` | `0` |
...
The True Delivery Loop MUST follow the canonical state sequence:

```
IDLE → COLLECT → ACCUMULATE → APPLY → CHECK → SEAL → DONE
```

No additional or intermediate states are permitted.
...
- The initial lattice contains a single asymmetric cell (`4096` in one position)  
- `symmetry_ok` MUST remain **0** during the entire `CHECK` cycle  
- No implementation may apply smoothing, tolerance, or normalization  
...
- `true_delivery` MUST remain **0**  
- Asymmetry alone is sufficient to invalidate the vector  

### **3.5 Seal Boundary**

- `seal_start` pulses high for exactly one cycle at SEAL entry  
- `seal_ready` asserts when SHA3‑256 completes  
- The seal encodes a **FALSE** result  
...
2. `symmetry_ok` remains `0` during CHECK  
3. `error_ok` transitions `0 → 1` during CHECK  
4. `true_delivery` remains `0`  
...
| Version | Date | Notes |
|---------|-------|--------|
| 0.1 | 2026‑01‑31 | Initial release of V0004 Golden Waveform template |